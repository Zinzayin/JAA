...
try:
    ...
    cursor.execute("select * from public.flight_flight where schedule_flight_time >= '2024-01-20 10:00:00' and canceled ='FALSE' and type ='DEP';")
    # Fetch result
...
current_time_utc = dt.utcnow() + timedelta(hours=2)
...
unique_tractors = pair['Tractor'].unique().tolist()
print('YYYYYYYYYYYYYY')
print(unique_tractors)
print(len(np.unique(pair['Tractor'].unique())))
...
max_GSE2 = mean_GSE2 + 3
# Mark 'Yes' for specified Tractor_ID from unique_tractors
Tractor.loc[Tractor['Tractor_ID'].isin(unique_tractors), 'Tractor_ava'] = 'Yes'

# Distribute remaining 'Yes' marks equally among 'No' entries based on 'Medium' and 'Large' size
remaining_yes_count = max_GSE2 + 2 - len(Tractor[Tractor['Tractor_ava'] == 'Yes'])
medium_large_no_indices = Tractor[Tractor['Tractor_ava'] == 'No'].index

# Determine the number of 'Yes' marks for Medium and Large size in a 50/50 ratio
medium_yes_count = remaining_yes_count // 2
large_yes_count = remaining_yes_count - medium_yes_count

# Randomly select 'No' indices for Medium and Large size
medium_no_indices = medium_large_no_indices[Tractor.loc[medium_large_no_indices, 'Tractor_size'] == 'Medium']
large_no_indices = medium_large_no_indices[Tractor.loc[medium_large_no_indices, 'Tractor_size'] == 'Large']

# Mark 'Yes' for Medium and Large size in a 50/50 ratio
Tractor.loc[random.sample(medium_no_indices.tolist(), medium_yes_count), 'Tractor_ava'] = 'Yes'
Tractor.loc[random.sample(large_no_indices.tolist(), large_yes_count), 'Tractor_ava'] = 'Yes'

# Print the updated dataframe
print('Update Tractor')
print(Tractor)
